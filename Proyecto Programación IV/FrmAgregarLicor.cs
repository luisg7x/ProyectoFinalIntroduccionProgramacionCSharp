...
{
    public partial class FrmAgregarLicor : Form
    {
        string[] rols = new string[6];
        string codigoValor = string.Empty;
        public FrmAgregarLicor()
        {
            InitializeComponent();
            codigo();
            textBoxUnitario.Enabled = false;
            textBoxBotella.Enabled = false;
            cargarComboMarca();
       ...
       private void buttonExit_Click(object sender, EventArgs e)
        ...
        {
            if (!checkBoxPrecioUnitario.Checked && !checkBoxPrecioBotella.Checked)
            {
                CustomMessageBox.ShowMessage("Debe seleccionar algun precio", "Validación de datos", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return true;
            }
            else
            {
                if (checkBoxPrecioUnitario.Checked && !checkBoxPrecioBotella.Checked)
                {
                    if (string.IsNullOrEmpty(textBoxNombre.Text) || pictureBox2.Image == null || string.IsNullOrEmpty(textBoxRestaurante.Text) || string.IsNullOrEmpty(textBoxCantidad.Text) || string.IsNullOrEmpty(textBoxDescripcion.Text) || string.IsNullOrEmpty(textBoxUnitario.Text))
                    {
                ...
                if (checkBoxPrecioBotella.Checked && !checkBoxPrecioUnitario.Checked)
                {
                    if (string.IsNullOrEmpty(textBoxNombre.Text) || pictureBox2.Image == null || string.IsNullOrEmpty(textBoxRestaurante.Text) || string.IsNullOrEmpty(textBoxCantidad.Text) || string.IsNullOrEmpty(textBoxDescripcion.Text) || string.IsNullOrEmpty(textBoxBotella.Text))
                    {
                        CustomMessageBox.ShowMessage("Posibles errores: \n• No se permiten numeros o no hay ningun valor en el campo: \n   Nombre, Restaurante..  o Precio esta vacia. \n• No ha cargado la fotografia", "Validación de datos", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return true;
                    }
        ...
        private void agregar()
        {
            ...
            if (validar() == false)
            {
                ...
                if (checkBoxPrecioBotella.Checked && !checkBoxPrecioUnitario.Checked)
                {
                    ...
                    cLogica_BD.marca = comboBoxMarca.SelectedItem.ToString();
                    cLogica_BD.nacionalidad = comboBoxNacionalidad.SelectedItem.ToString();
                    cLogica_BD.precio = Convert.ToInt32(textBoxBotella.Text);
                    cLogica_BD.restaurante = textBoxRestaurante.Text;
                    cLogica_BD.cantidad = textBoxCantidad.Text;
                    cLogica_BD.descripcion = textBoxDescripcion.Text;
                    cLogica_BD.foto = imageToByteArray(pictureBox2.Image);
               ...
               if (cLogica_BD.agregar_Licor() == true)
                {
                    Bitacora("Se agrego un Licor", "Se agrego un Licor");
        ...
        private void codigo()
        {
            ...
            string[] valores = clogiga.CodigoTabla("CodigoLicor", "Licores");
        ...
        private void checkBoxPrecioUnitario_Click(object sender, EventArgs e)
        {
            if (checkBoxPrecioUnitario.Checked)
            {
        ...
        private void checkBoxPrecioBotella_Click(object sender, EventArgs e)
        {
            if (checkBoxPrecioBotella.Checked)
            {
                textBoxBotella.Enabled = true;
            }
            else
            {
                textBoxBotella.Enabled = false;
