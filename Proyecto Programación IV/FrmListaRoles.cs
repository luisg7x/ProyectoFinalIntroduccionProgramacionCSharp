...
namespace Proyecto_Programación_IV
{
    public partial class FrmListaRoles : Form
    {
        ...
        {
            InitializeComponent();
            actualizarDataGrid("select CodigoRol,NombreRol from Roles");
        ...
        private void dataGridView1_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
           ...
           bd.executecommand("update Roles set NombreRol='" + clogiga.encriptacion(dataGridView1.Rows[e.RowIndex].Cells[1].Value.ToString()) + "'WHERE CodigoRol='" + clogiga.encriptacion(id) + "'");
           Bitacora("Actualizo un Rol", "Actualizo un Rol");
        ...
        private void buttonBuscar_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(TextboxCodigo.Text) && string.IsNullOrEmpty(TextboxNombre.Text)) actualizarDataGrid("Select CodigoRol,NombreRol from Roles where CodigoRol= '" + clogiga.encriptacion(TextboxCodigo.Text) + "'");
            if (!string.IsNullOrEmpty(TextboxNombre.Text) && string.IsNullOrEmpty(TextboxCodigo.Text)) actualizarDataGrid("Select CodigoRol,NombreRol from Roles where NombreRol= '" + clogiga.encriptacion(TextboxNombre.Text) + "'");
            if (!string.IsNullOrEmpty(TextboxNombre.Text) && !string.IsNullOrEmpty(TextboxCodigo.Text)) actualizarDataGrid("Select CodigoRol,NombreRol from Roles where NombreRol= '" + clogiga.encriptacion(TextboxNombre.Text) + "' and CodigoRol='" + clogiga.encriptacion(TextboxCodigo.Text) + "'");
        ...
        {  
            bd.autocompletar("select NombreRol from Roles", "NombreRol", 1, TextboxNombre);
            bd.autocompletar("select CodigoRol from Roles", "CodigoRol", 1, TextboxCodigo);
        }
        ...
        {
            FrmAgregarRol frm = new FrmAgregarRol();
            frm.Show();
        }
        ...
        private void buttonEliminar_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow row in dataGridView1.SelectedRows) 
            {
                ...
                bd.executecommand("delete Roles WHERE CodigoRol='" + clogiga.encriptacion(id) + "'");
                actualizarDataGrid("select CodigoRol,NombreRol from Roles");
                Bitacora("Elimino un Rol", "Elimino un Rol");
